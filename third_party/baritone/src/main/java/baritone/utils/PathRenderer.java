...
public final class PathRenderer implements IRenderer {
    ...
    public static void render(RenderEvent event, PathingBehavior behavior) {
        ...
        if (ctx.minecraft().level == null || ctx.minecraft().player == null
                || ctx.world() == null || ctx.player() == null
                || ctx.minecraft().level != ctx.world()) {
            return;
        }
        ...
        final IPlayerContext renderContext = BaritoneAPI.getProvider().getPrimaryBaritone().getPlayerContext();
        if (renderContext.world() == null || renderContext.player() == null) {
            return;
        }
        final DimensionType thisPlayerDimension = ctx.world().dimensionType();
        final DimensionType currentRenderViewDimension = renderContext.world().dimensionType();
    ...
    public static void drawPath(PoseStack stack, List<BetterBlockPos> positions, int startIndex, Color color, boolean fadeOut, int fadeStart0, int fadeEnd0, double offset) {
        if (positions == null || startIndex < 0 || startIndex >= positions.size() - 1) {
            return;
        }

        try (IRenderer.RenderBatch batch = IRenderer.startLines(color)) {
            BufferBuilder bufferBuilder = batch.bufferBuilder();
            ...
            IRenderer.endLines(batch, settings.renderPathIgnoreDepth.value);
        }
    }
    ...
    public static void drawManySelectionBoxes(PoseStack stack, Entity player, Collection<BlockPos> positions, Color color) {
        ...
        try (IRenderer.RenderBatch batch = IRenderer.startLines(color)) {
            ...
            IRenderer.endLines(batch, settings.renderSelectionBoxesIgnoreDepth.value);
        }
    }
    ...
    private static void drawGoal(@Nullable BufferBuilder bufferBuilder, PoseStack stack, IPlayerContext ctx, Goal goal, float partialTicks, Color color, boolean setupRender) {
        ...
        if (goal instanceof IGoalRenderPos) {
            ...
            if (batch) {
                try (IRenderer.RenderBatch renderBatch = IRenderer.startLines(color)) {
                    BufferBuilder buf = renderBatch.bufferBuilder();
                    for (Goal g : ((GoalComposite) goal).goals()) {
                        drawGoal(buf, stack, ctx, g, partialTicks, color, false);
                    }
                    IRenderer.endLines(renderBatch, settings.renderGoalIgnoreDepth.value);
                }
            } else {
                for (Goal g : ((GoalComposite) goal).goals()) {
                    drawGoal(bufferBuilder, stack, ctx, g, partialTicks, color, true);
                }
            }
    ...
    private static void drawDankLitGoalBox(BufferBuilder bufferBuilder, PoseStack stack, Color colorIn, double minX, double maxX, double minZ, double maxZ, double minY, double maxY, double y1, double y2, boolean setupRender) {
        if (setupRender) {
            try (IRenderer.RenderBatch batch = IRenderer.startLines(colorIn)) {
                drawDankLitGoalBox(batch.bufferBuilder(), stack, colorIn, minX, maxX, minZ, maxZ, minY, maxY, y1, y2, false);
                IRenderer.endLines(batch, settings.renderGoalIgnoreDepth.value);
            }
            return;
        }
    ...
    private static void renderGoalXZBeaconLayer(PoseStack stack, double height, float time, int color, float radius, boolean translucent) {
        try (IRenderer.RenderBatch batch = IRenderer.startBlockQuads()) {
            BufferBuilder bufferBuilder = batch.bufferBuilder();
            ...
            try {
                ...
                if (!translucent) {
                    ...
                    try {
                        ...
                        emitGoalBeaconGeometry(bufferBuilder, stack, height, color, radius, translucent, scroll);
                    } finally {
                        stack.popPose();
                    }
                } else {
                    emitGoalBeaconGeometry(bufferBuilder, stack, height, color, radius, true, scroll);
                }
            } finally {
                stack.popPose();
            }

            IRenderer.endBuffer(batch, IRenderer.beaconBeam(BeaconRenderer.BEAM_LOCATION, translucent, settings.renderGoalIgnoreDepth.value));
        }
    }

    private static void emitGoalBeaconGeometry(BufferBuilder bufferBuilder, PoseStack stack, double height, int color, float radius, boolean translucent, float scroll) {
