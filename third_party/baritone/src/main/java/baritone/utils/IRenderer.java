...
public interface IRenderer {
    ...
    int INITIAL_BUFFER_SIZE = 64 * 1024;
    ...
    static RenderBatch startLines(Color color, float alpha) {
        glColor(color, alpha);
        return new RenderBatch(PrimitiveTopology.LINES, DefaultVertexFormat.POSITION_COLOR_NORMAL_LINE_WIDTH);
    }

    static RenderBatch startLines(Color color) {
        return startLines(color, .4f);
    }

    static void endLines(RenderBatch batch, boolean ignoredDepth) {
        batch.draw(ignoredDepth ? linesNoDepthRenderType : linesWithDepthRenderType);
    }

    static RenderBatch startBlockQuads() {
        return new RenderBatch(PrimitiveTopology.QUADS, DefaultVertexFormat.BLOCK);
    }

    static void endBuffer(RenderBatch batch, RenderType renderType) {
        batch.draw(renderType);
    }

    static void drawMesh(MeshData meshData, RenderType renderType) {
        ...
        try {
            vertexBuffer = device.createBuffer(() -> "Baritone overlay vertices", GpuBuffer.USAGE_VERTEX, meshData.vertexBuffer());
            ByteBuffer meshIndexBuffer = meshData.indexBuffer();
            IndexType indexType;
            if (meshIndexBuffer != null) {
                indexBuffer = device.createBuffer(() -> "Baritone overlay indices", GpuBuffer.USAGE_INDEX, meshIndexBuffer);
    ...
    /** Owns the native allocator that backs one immediate-mode Baritone draw. */
    final class RenderBatch implements AutoCloseable {
        ...
        private RenderBatch(PrimitiveTopology primitiveTopology, VertexFormat format) {
            ByteBufferBuilder newAllocator = new ByteBufferBuilder(INITIAL_BUFFER_SIZE);
            BufferBuilder newBufferBuilder;
            try {
                newBufferBuilder = new BufferBuilder(newAllocator, primitiveTopology, format);
            } catch (RuntimeException | Error failure) {
                newAllocator.close();
                throw failure;
            }
            this.allocator = newAllocator;
            this.bufferBuilder = newBufferBuilder;
            RenderDiagnostics.onBufferAllocated(INITIAL_BUFFER_SIZE);
        }

        public BufferBuilder bufferBuilder() {
            if (closed) {
                throw new IllegalStateException("Render batch is already closed");
            }
        ...
        private void draw(RenderType renderType) {
            if (built) {
                throw new IllegalStateException("Render batch was already built");
            }
            built = true;
            MeshData meshData = bufferBuilder.build();
            if (meshData != null) {
                try (meshData) {
                    RenderDiagnostics.onMeshDrawn(meshData.drawState().vertexCount());
                    IRenderer.drawMesh(meshData, renderType);
                }
            }
        }
        ...
        public void close() {
            ...
            try {
                ...
                RenderDiagnostics.onBufferClosed(INITIAL_BUFFER_SIZE);
            }
        }
    }
}
