...
public final class ElytraBehavior implements Helper {
    ...
    public void onRenderPass(RenderEvent event) {
        ...
        if (!this.clearLines.isEmpty() && settings.elytraRenderRaytraces.value) {
            try (IRenderer.RenderBatch batch = IRenderer.startLines(Color.GREEN)) {
        ...
        if (!this.blockedLines.isEmpty() && Baritone.settings().elytraRenderRaytraces.value) {
            try (IRenderer.RenderBatch batch = IRenderer.startLines(Color.BLUE)) {
                BufferBuilder bufferBuilder = batch.bufferBuilder();
        ...
        if (this.simulationLine != null && Baritone.settings().elytraRenderSimulation.value) {
            try (IRenderer.RenderBatch batch = IRenderer.startLines(new Color(0x36CCDC))) {
                BufferBuilder bufferBuilder = batch.bufferBuilder();
    ...
    public void destroy() {
        ...
        try {
            terminated = this.solverExecutor.awaitTermination(5, TimeUnit.SECONDS);
        ...
        if (terminated) {
            ...
            System.err.println("Baritone Elytra solver did not terminate; retaining its native context");
        }
    }
    ...
    public void onTick() {
        ...
        if ((now - this.timeLastCacheCull) / 1000 > Baritone.settings().elytraTimeBetweenCacheCullSecs.value) {
            ChunkPos chunkPos = ctx.player().chunkPosition();
            this.context.queueCacheCulling(chunkPos.x(), chunkPos.z(), Baritone.settings().elytraCacheCullDistance.value, this.boi);
